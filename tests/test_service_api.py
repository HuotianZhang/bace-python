...
# -- the SMU panel ---------------------------------------------------------------------------
def test_the_smu_panel_sources_reads_and_refuses_over_the_ceiling(service):
    """The Keithley by itself: three actions and the existing `smu-off`, with
    no run and no file behind any of them. What the route layer owes is the
    status code -- a bench that will not do this *now* is a 409 with the
    sentence, and a body it cannot read is a 422 that names the field."""
    client, session = service
    smu = client.get("/bench").json()["instruments"]["smu"]
    assert smu["panel"] is None and smu["panel_defaults"]["function"] == "voltage"

    assert client.post("/bench/actions/smu-on", json={}).status_code == 409

    r = client.post("/bench/actions/smu-source",
                    json={"function": "I", "level": 0.0, "voltage_compliance_v": 2.0})
    assert r.status_code == 202, r.text
    assert r.json()["result"]["panel"]["function"] == "current"
    assert r.json()["bench"]["instruments"]["smu"]["panel"]["unit"] == "A"

    over = client.post("/bench/actions/smu-source", json={"current_compliance_a": 0.5})
    assert over.status_code == 409 and over.json()["level"] == "crit"
    assert "max_current_compliance_a" in over.json()["error"]
    bad = client.post("/bench/actions/smu-source", json={"nplc": 50})
    assert bad.status_code == 422 and "nplc" in bad.json()["error"]

    client.post("/bench/actions/relay-to-sourcemeter", json={})
    assert client.post("/bench/actions/smu-on", json={}).status_code == 202
    assert client.get("/bench").json()["instruments"]["smu"]["output"] is True
    reading = client.post("/bench/actions/smu-read", json={}).json()["result"]
    assert reading["function"] == "current" and reading["level"] == 0.0
    assert reading["volts"] is not None and reading["compliance"] is False
    # By hand, so it is in the journal as the action that took it -- the same
    # shape `read-power` has, and not an event of its own.
    assert any(l["type"] == "BenchAction" and l["data"]["name"] == "smu-read"
               for l in journal_lines(session))
    assert client.post("/bench/actions/smu-off", json={}).status_code == 202


def test_a_run_takes_the_panel_away_before_anybody_reads_the_bench_back(service):
    """The bug this is here for: `GET /bench` serves the read-back **Start**
    took, and a run does not read back at its end. So a panel the run's first
    `*RST` had already cleared went on being served as applied until somebody
    pressed re-read -- and the console would have offered a switch-on that the
    instrument would have answered at 0 V on its reset compliance.

    The fix is not another read-back: the panel is not a read-back at all. It
    is the driver's own record of what this process configured, no instrument
    is touched to read it, so the snapshot lays the live one over its cached
    copy (`Bench.smu_panel_block`).
    """
    client, session = service
    client.post("/bench/actions/relay-to-sourcemeter", json={})
    client.post("/bench/actions/smu-source", json={"function": "I", "level": 0.0})
    assert client.get("/bench").json()["instruments"]["smu"]["panel"] is not None

    run = client.post("/runs", json={"module": "jv"}).json()["run_id"]
    wait_until(lambda: client.get(f"/runs/{run}").json()["state"] == "done")
    assert session.bench.rig.smu.panel is None, "the sweep's *RST cleared it"
    assert client.get("/bench").json()["instruments"]["smu"]["panel"] is None, \
        "and the snapshot says so without waiting for a read-back"
    # `panel_defaults` is computed, not read, so it survives -- the panel can
    # be applied again from the same starting point.
    assert client.get("/bench").json()["instruments"]["smu"]["panel_defaults"] is not None
    assert client.post("/bench/actions/smu-on", json={}).status_code == 409


def test_the_smu_monitor_free_runs_the_display_and_skips_what_it_cannot_read(service):
    """The panel's display: a thread in the *service*, so it goes on reading
    with the browser closed. It reads only with the output on and the panel
    applied -- neither is a failure and neither is a number, so a tick without
    them is a skip and never a `warn` saying the instrument stopped
    answering."""
    client, session = service
    r = client.post("/monitors/smu", json={"interval_s": 0.01})
    assert r.status_code == 202 and r.json()["running"] is True
    assert client.post("/monitors/smu", json={"interval_s": 0.5}).status_code == 409
    assert {m["name"] for m in client.get("/monitors").json()["monitors"]} == {"smu"}

    wait_until(lambda: monitor_of(client, "smu")["skipped"] >= 3)
    assert monitor_of(client, "smu")["readings"] == 0
    assert monitor_of(client, "smu")["failures"] == 0, "an output that is off is not a fault"

    client.post("/bench/actions/relay-to-sourcemeter", json={})
    client.post("/bench/actions/smu-source", json={"function": "I", "level": 0.0})
    client.post("/bench/actions/smu-on", json={})
    wait_until(lambda: sum(1 for f in frames(client) if f["type"] == "SmuReading") >= 3)
    reading = next(f for f in frames(client) if f["type"] == "SmuReading")
    assert reading["run_id"] is None and reading["node_path"] == ""
    assert reading["data"]["function"] == "current" and reading["data"]["level"] == 0.0
    assert reading["data"]["volts"] is not None

    r = client.delete("/monitors/smu")
    assert r.status_code == 200 and r.json()["stopped"] is True
    assert client.delete("/monitors/smu").status_code == 404
    assert client.get("/bench").json()["instruments"]["smu"]["output"] is True, \
        "stopping a display is not stopping a source"
    assert client.post("/monitors/smu", json={"interval_s": 0}).status_code == 422


def monitor_of(client, name):
    return next(m for m in client.get("/monitors").json()["monitors"] if m["name"] == name)
