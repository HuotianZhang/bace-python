...
# -- the SMU panel ------------------------------------------------------------
# The Keithley on its own: `smu-source`, `smu-on`, `smu-read` and the existing
# `smu-off`. No module, no run, no file -- and, because it is the one surface
# on this bench where a level is typed straight onto the device, the only one
# where `rig.toml`'s ceilings have to hold a *level* and not just a compliance.
def test_the_panel_opens_on_the_bench_configuration_and_is_not_applied_yet():
    b = bench()
    smu = b.read_back()["instruments"]["smu"]
    assert smu["panel"] is None, "nothing has told the instrument what to source"
    defaults = smu["panel_defaults"]
    assert defaults["function"] == "voltage" and defaults["level"] == 0.0
    assert defaults["unit"] == "V" and defaults["limit"] == defaults["current_compliance_a"]
    assert defaults["current_compliance_a"] <= smu["ceiling"]["current_a"]
    assert defaults["voltage_compliance_v"] <= smu["ceiling"]["voltage_v"]


def test_the_panel_defaults_never_exceed_the_bench_ceiling():
    ...
    rig = RigConfig(max_voltage_compliance_v=1.0, max_current_compliance_a=0.002)
    b = Bench.build_simulated(rig, seed=1)
    b.rig.smu.config = SourceMeterConfig(current_compliance_a=0.05,
                                         voltage_compliance_v=2.0)
    setup = b.panel_defaults()
    assert (setup.current_compliance_a, setup.voltage_compliance_v) == (0.002, 1.0)


def test_smu_source_merges_onto_the_panel_the_instrument_holds():
    """Every field optional: moving a level is a body of one key, and the rest
    of the panel is what the instrument is already in rather than whatever a
    form last posted."""
    b = bench()
    first = b.action("smu-source", {"function": "I", "voltage_compliance_v": 1.5})
    assert first["panel"]["function"] == "current" and first["panel"]["unit"] == "A"
    assert first["panel"]["limit"] == 1.5, "sourcing amps, the volt limit is the one that bites"
    assert first["output"] is False, "setting a source does not drive anything"

    moved = b.action("smu-source", {"level": 1e-4})
    assert moved["panel"]["level"] == 1e-4
    assert moved["panel"]["voltage_compliance_v"] == 1.5, "unchanged, not reset"
    assert moved["panel"]["function"] == "current"


def test_the_ceiling_holds_a_typed_level_and_not_only_a_compliance():
    """Until this panel nothing could type a level at all -- a sweep's ends
    come from a module's parameters and V_oc/J_sc source zero. `rig.toml`'s
    two keys are the bench's statement of what the device may see, whichever
    end of the instrument it arrives from, so they hold here too."""
    b = bench()                                     # 0.05 A / 5.0 V
    with pytest.raises(BenchActionRefused) as over_v:
        b.action("smu-source", {"function": "V", "level": 9.0})
    assert over_v.value.level == "crit" and "max_voltage_compliance_v" in over_v.value.text

    b.action("smu-source", {"function": "I"})
    with pytest.raises(BenchActionRefused, match="max_current_compliance_a"):
        b.action("smu-source", {"level": 0.5})
    with pytest.raises(BenchActionRefused, match="max_current_compliance_a"):
        b.action("smu-source", {"current_compliance_a": 0.5})
    with pytest.raises(BenchActionRefused, match="max_voltage_compliance_v"):
        b.action("smu-source", {"voltage_compliance_v": 9.0})
    assert b.rig.smu.panel.level == 0.0, "nothing refused reached the instrument"


def test_the_panel_body_is_typed_and_a_bad_value_names_its_field():
    b = bench()
    for body, message in (({"nplc": "fast"}, "nplc: 'fast' is not a number"),
        ...
        with pytest.raises(ValueError, match=message.replace("'", "'")):
            b.action("smu-source", body)
    with pytest.raises(ValueError, match="unknown argument"):
        b.action("smu-source", {"knob": 1})
    # A form's empty box is the instrument's autorange, not a zero range.
    assert b.action("smu-source", {"source_range": ""})["panel"]["source_range"] is None
    assert b.action("smu-source", {"function": "v"})["panel"]["function"] == "voltage"


def test_the_output_will_not_switch_on_until_something_has_set_a_source():
    """After a `*RST` -- which every measurement routine opens with -- the 2400
    is at 0 V on a 100 uA compliance. An output switched on there is not the
    source the panel is drawing."""
    b = bench()
    with pytest.raises(BenchActionRefused) as exc:
        b.action("smu-on", {})
    assert exc.value.level == "warn" and "smu-source" in exc.value.text
    assert b.sim.bench.smu_output is False

    b.action("smu-source", {"function": "I", "level": 0.0})
    assert b.action("smu-on", {})["output"] is True
    assert b.sim.bench.smu_output is True
    assert b.action("smu-off", {}) == {"output": False}


def test_a_by_hand_reading_is_the_open_circuit_voltage_and_needs_the_output_on():
    b = bench()
    b.action("relay-to-sourcemeter", {})
    b.action("shutter-open", {})
    b.action("set-led-dc", {"level": 1.02})
    b.action("smu-source", {"function": "I", "level": 0.0, "voltage_compliance_v": 2.0})
    with pytest.raises(BenchActionRefused, match="output is OFF"):
        b.action("smu-read", {})
    b.action("smu-on", {})
    reading = b.action("smu-read", {})
    assert 0.8 < reading["volts"] < 1.0, "sourcing 0 A is V_oc, and the device is lit"
    assert reading["compliance"] is False and reading["function"] == "current"
    assert reading["at"] > 0


def test_off_the_relay_the_panel_reads_an_open_circuit():
    """The relay is not moved from this panel and still decides what it reads.
    The simulated SourceMeter models that on the panel path -- and only there:
    `measure_dc` and `sweep_points` are reached from inside `router.dc()`,
    which has already connected the instrument, so the position cannot be
    wrong for them."""
    b = bench()
    b.action("shutter-open", {})
    b.action("set-led-dc", {"level": 1.02})
    b.action("smu-source", {"function": "V", "level": 0.5})
    b.action("smu-on", {})
    assert b.sim.bench.relay == "amplifier"
    off = b.action("smu-read", {})
    assert off["volts"] == 0.5 and abs(off["amps"]) < 1e-5, "no device on this side"

    b.action("smu-off", {})
    b.action("relay-to-sourcemeter", {})
    b.action("smu-on", {})
    on = b.action("smu-read", {})
    assert on["amps"] < -1e-5, "a lit cell at 0.5 V pushes current the other way"


def test_the_compliance_light_is_the_instrument_saying_it_is_clamping():
    b = bench()
    b.action("relay-to-sourcemeter", {})
    b.action("shutter-open", {})
    b.action("set-led-dc", {"level": 1.02})
    b.action("smu-source", {"function": "V", "level": 1.5, "current_compliance_a": 0.05})
    b.action("smu-on", {})
    reading = b.action("smu-read", {})
    assert reading["compliance"] is True
    assert abs(reading["amps"]) == pytest.approx(0.05)
    assert reading["volts"] < 1.5, "the source cannot hold its level at that current"


def test_a_run_takes_the_panel_away_and_the_read_back_says_so():
    """`Keithley2400._prepare` clears the panel because `*RST` clears the
    instrument; the simulated one does the same, so the console develops
    against the bench it will meet."""
    b = bench()
    b.action("relay-to-sourcemeter", {})
    b.action("smu-source", {"function": "I", "level": 0.0})
    assert b.read_back()["instruments"]["smu"]["panel"] is not None
    b.rig.smu.measure_dc(v_sat=-1.0)
    assert b.read_back()["instruments"]["smu"]["panel"] is None
