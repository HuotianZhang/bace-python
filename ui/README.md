...
| `lib/format.js` | the number rules of `docs/ui-rules.md` §2, including the two zeros that are absences — and, since #68, the minus sign: `MINUS` is U+2212 and lives here rather than in `scale.js`, because it is a rule about how a *number* is spelled and not about how an *axis* is drawn (the axis was simply the only place that had one, which is how the tables came to disagree with it). `minus` converts a number this module built; `minusIn` converts only the signs in a sentence it did not — `outcome_text` comes from `service/journal.py` already composed, and the same field carries `failed: <path>`. `label`, `keptOf` and `plural` render names and counts and are left alone |
...
| `minus-glyph.html` | 负号的两个字形，量过再决定 (2026-09-06, #68): the two glyphs at every face and size the console uses, a column of readings rendered both ways, and the four fixes with the census behind each. Advance widths and ink boxes are measured in the browser at load, never typed. It settles a claim the issue made and the reason it gave: in **IBM Plex Mono the column does not move at all** — `-` and `−` advance identically at all four sizes — so there the defect is two glyphs for one meaning on one card; the jitter `ui-rules` §2 names is real in **Sans and Archivo**, where a hyphen is 1.8–3.5 px narrower than a digit and a real minus is exactly a digit wide, which is where the biggest site on screen happens to be |
...
| `fixtures/` | see below |
| `tests/minus.test.mjs` | the two roads a number takes (#68): a reader's is U+2212 — the axis, the metric table, the rail, the shot rows, the schedule, the drift note — and a machine's is U+002D — an `<input>` the operator types back (`Number('−0.2')` is `NaN`), the keys the results grid finds a cell by, the CSV, a file stem. The boundary is what is under test, not the glyph; `tools/minus_census.py` counts what actually reaches the screen |
