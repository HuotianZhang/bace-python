...
| `lib/rail.js` | the pinned rail and the chain strip: `railModel(state)` is a pure function of the store, the DOM is beside it. `temperatureNow(state)` lays the newest `TemperatureRead` over the read-back's block, so the T cell follows the monitor the service runs from boot rather than the last `/bench`; the sub-line carries the monitor's cadence, and a monitor that has not read for three of its intervals reads `stale` and loses its band verdict — `in band` is a claim about now. The optical-power cell takes `power.newestReading` — the newest of the stream's frame and the read-back — because the read-back that follows a bench action is taken before the meter settles, and reading it alone put `0 W` in the rail above `1.70 mW` in the panel; it also carries the trace itself (`charts/power.sparkModel`, 60 × 18) and the handle for the panel's drawer. The spark is drawn *beside* the keyed rail: a `PowerReading` arrives five times a second and in the key it would tear the row — and the handle under the pointer — down at that rate |
...
| `lib/power.js`, `lib/charts/power.js` | the power panel (2026-09-05): the 1918-C in a drawer under the rail, opened from the rail's own cell — the switch that starts and stops the monitor *in the service* and is remembered here, so "on" is re-asserted on every connect; the reading with what the meter said about it (averaged, wavelength, saturated, stale); the trace, from the store's `powerLog` (every `PowerReading`, merged by `ts` with `GET /monitors/power/history` on connect), over a window that is `auto` by default — everything, until two hours. Off, the panel is one line — the switch, the last spot check muted, and the `⋯`, which off carries the interval, Export CSV and Clear, so that switching the monitor off never takes away what it recorded; on, the reading and the trace, and the `⋯` gains the window, the chart toggles and Export SVG (the chart on screen). `powerPanelModel` is pure, and so are the two decisions beside it — `panelShape` (what is drawn) and `panelKeys` (what each part is rebuilt on, which is where a key carrying the reading count tore the `⋯` down five times a second); `ui/tests/power.test.mjs` holds all three down. **Shut is the default** (2026-09-05): the panel was 263 px of a 1080 px screen on all four tabs — 19 % fixed chrome to 41 % the moment the monitor was switched on — and most of the time it drew one straight line and `mean 0 W · σ 0 W`. The rail's cell now carries the number and `sparkModel`'s 60 × 18 trace of the last two minutes, and the handle beside it opens the panel whole; the wish is remembered like the monitor's own. Shut, `renderPowerPanel` returns before its keyed parts, so a reading arriving five times a second rebuilds nothing |
