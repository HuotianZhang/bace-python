...
| `jv-width.html` | J–V 槽位里那一列 172 px，改前改后并排 (2026-09-06, #64): the same `jv` card from the same fixtures through the same `lib/card.js` and `lib/results.js`, twice — A re-asserts the pre-#64 rule through a `.v-old` class, B is `style.css` as it stands, and nothing else differs. The `bace` card sits underneath as the control, because it is the one panel that does own a `.shot` and must not move. Every length is read off the live layout at five viewport widths and divided back out of the display scale, so the numbers on the page are the console's: at 1920 the drawing goes 384 → 570 px of a 570 px slot, its x-axis labels from 24.4 px apart to 50.0, and a decade of the log axis from 14.9 px to 23.9 — and the `bace` card does not move by a pixel |
