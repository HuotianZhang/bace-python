...
| `runs_sim.json` | `GET /runs?session=all`: three rows, each with the `[sample]` block it was queued under — the identity per run, never a folder name parsed (`docs/history/naming-plan.md` rule 1) |
...
* **Nothing here is computed from a folder name, and no HDF5 is opened.** The
  record carries what a grid needs, because M6 is what `docs/history/naming-plan.md`
...
* **The temperature row says how it knows.** `how` and `source` both, as
  `docs/history/ui-kickoff.md` asks: *typed by the operator at the pause* is not
...
`studies/bench-head.html` is the four layouts measured side by side. **B shipped**: the
