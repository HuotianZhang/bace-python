...
| | |
|---|---|
| `index.html`, `app.js`, `style.css` | the shell: the bar and its chips, the rail, five tabs behind a hash router, the chain strip, the stream bar |
...
| `lib/power.js`, `lib/charts/power.js` | the power panel (2026-09-05): the 1918-C under the rail on every tab — the switch that starts and stops the monitor *in the service* and is remembered here, so "on" is re-asserted on every connect; the reading with what the meter said about it (averaged, wavelength, saturated, stale); the trace, from the store's `powerLog` (every `PowerReading`, merged by `ts` with `GET /monitors/power/history` on connect), over a window that is `auto` by default — everything, until two hours. Off, the panel is one line — the switch, the last spot check muted, and the `⋯`, which off carries the interval, Export CSV and Clear, so that switching the monitor off never takes away what it recorded; on, the reading and the trace, and the `⋯` gains the window, `y from 0` and Export SVG (the chart on screen). The trace folds away on its own (2026-09-06, `▾ trace` beside the `⋯`, remembered): the switch is about the bench and the fold is about the screen, so an operator who wants the 210 px back does not have to stop the service reading the meter and put a hole in the run's power record. `powerPanelModel` is pure, and so are the two decisions beside it — `panelShape` (what is drawn) and `panelKeys` (what each part is rebuilt on, which is where a key carrying the reading count tore the `⋯` down five times a second); `ui/tests/power.test.mjs` holds all three down |
| `lib/smu.js` | the SMU panel (2026-09-07): the Keithley 2400 by itself — source a voltage or a current, hold a compliance, switch the output on, watch what comes back. It touches the SourceMeter and nothing else: no module, no run, no folder, and no other instrument. Every control is the switch family, because every one of them is a bench action the moment it lands (`smu-source`, `smu-on`/`smu-off`, `smu-read`, and `POST /monitors/smu` for the free-running display, which runs in the *service* so it goes on reading with the tab closed). Four things it owes that the instrument's own panel does not: **the display is dashes unless there is something to display** and always says which of the three reasons it is (not on the panel / output off / nothing read yet) — with the output off a `:READ?` still answers, from a source disconnected inside the instrument, and the near-zero it returns looks exactly like a measurement of a dead device; **a run takes the instrument away**, because every measurement routine opens with `*RST`, so the panel comes back unapplied and the service says so rather than this deciding; **the relay is not moved from here** and still decides what the panel reads, so the position is stated and the bench tab is named (a second surface for the one action on this rig that can damage hardware is not reach, it is a second place to hit by accident — `ui-rules` §14's argument for keeping Park on the strip alone); and **`rig.toml`'s two ceilings hold a typed level**, not only a compliance, because until this panel nothing could type a level at all. `smuPanelModel` and `panelKeys` are pure and `ui/tests/smu.test.mjs` holds them down — including the rule that nothing moving with a reading may be in the controls' render key, or the fields cannot be typed into |
| `lib/identity.js` | what is mounted (2026-09-05, `docs/ux-screening.md` finding 5): the session's `[sample]` block, typed from the bar's own chip through `PUT /session/sample`. It draws the folder stem the next run will be filed under rather than five bare inputs — the operator is typing a filename — warns on the underscore and the space `naming-plan.md` says break one, and does not offer `temperature_k`, which `run.toml` says in a comment nobody should type. `identityModel` is pure and `ui/tests/identity.test.mjs` holds it down |
...
| `keithley.html` | the same panel in a window of its own (2026-09-07): a socket, a store and `lib/smu.js`. `docs/service-plan.md` already has this shape for the two instruments with their own consoles — 既有的独立 console 保持独立, the 1918-C on :8918 and the 331 on :8331 — and this is it for the one instrument the service itself owns, for an operator at the probe station who wants the Keithley on screen without the console's other tabs and a rail of instruments they are not using. The *same* service and the *same* bench lock, not a second client with its own ideas: `views/smu.js` exports the panel's actions and both hosts use them, so the console and this window cannot drift |
...
| `views/` | bench is M2's six generated cards with M3's charts in three of them, the one-row Instruments panel above them and the `MODULES` header between the two, which carries the saved bench and the `⋯` that saves, opens and deletes them (below); pipeline is M5's editor and schedule; results is M6's grid, drawn from `GET /runs` and `GET /runs/{id}` and nothing else; smu is the Keithley panel and its actions, shared with `keithley.html`; rig is R3·4: the chain, one shot at three scales with the swept axis taken from the `bace` card, and the read-back under them |
...
**The SMU panel is built** (2026-09-07): a fifth tab, `keithley`, and
`keithley.html` beside it — the Keithley 2400 driven by hand, with nothing
else on the surface. It is not a milestone in `docs/ui-plan.md`; it is the
thing an operator at a probe station asks for, and the thing the console
could not do at all: every route to the SourceMeter went through a module,
so measuring a V_oc without filing a run meant leaving the console for the
instrument's own keys. `bace/drivers/keithley2400.py` grew a panel mode for
it (configure once, read many, `*RST` only on the way in) and the service
three bench actions and a third observer; the console half is `lib/smu.js`.
