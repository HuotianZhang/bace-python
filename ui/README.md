...
| `jv-direction.html` | J–V 正扫 / 回扫的名字，四种改法并排 (2026-09-06, issue #66): as shipped, `· reverse` inside `curveLabel`, a `direction` column on the metrics table, and `→`/`←` on both arms when both are on the chart. Every number is measured on the live console — `--sim --fast`, one `jv` run with `both_directions`, each candidate a real edit to `lib/charts/jv.js`, read off the rendered DOM. **The page's own finding is that a curve is named in four places, not three**: the legend is right, and the metrics table, the crosshair readout and the direct label are all `as found 1.02 V` twice — the direct label additionally running 64 px outside the plot, because it is anchored `start` at `voltage[0]`, which for the reverse arm is the frame's right edge |
