...
| `jv-note.html` | J–V 脚注的四种写法，并排 (2026-09-06, #65): `pixel_area_cm2 = 0` 切轴到安培时，`notes()` 里写死的 `|J|` 没跟着切 — 轴标、「这次没有像素面积」和那一句，一张卡三句话两个量纲。四格用的是真的 `jvModel()` 和真的 `style.css`，四者之间只有那一句不同，宽度取控制台里 J–V 卡真正拿到的 380 px（1440 / 1920 双栏，在 `preview.html` 上量的；1280 单栏是 692）。一致性审计是 18 个状态当场跑的，余量和高度是浏览器里量的。**C 合入了** — 一致性 0 / 18，余量 130 px 和现状持平，而 B 在有面积时只剩 72、D 只剩 81。页脚记着 B / D 各自付了什么，以及真正拦住下一次的那条不变量：同样三个变异体打 HEAD 的 57 个测试一个都不红，加上不变量后三个全红 |
